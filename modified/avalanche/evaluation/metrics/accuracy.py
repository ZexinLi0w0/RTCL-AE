...
'''
Warning! This global variable should be set to True if and only if
the metric is used for semantic segmentation tasks.
'''
is_semseg_acc = False
...
class Accuracy(Metric[float]):
    ...
    def update(
        ...
        if is_semseg_acc:
            # change logits [N, C, H, W] to labels [N, H, W]
            ...
            total_patterns = true_y.numel()  # get pixel count
        else:
